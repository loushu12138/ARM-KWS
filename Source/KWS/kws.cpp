...
void KWS::extract_features() 
{
  ...
  const int16_t audio_buffer[16000]=UP;
  int32_t mfcc_buffer_head = 0;//(num_frames-recording_win)*num_mfcc_features; 
  for (uint16_t f = 0; f <25; f++) {
    mfcc->mfcc_compute(audio_buffer+(f*frame_shift),&mfcc_buffer[mfcc_buffer_head]);

    mfcc_buffer_head += num_mfcc_features;
  }
  // for (int i=0;i<250;i++){
  //     if(mfcc_buffer[i]>= 31.75)
  //     mfcc_buffer[i] = 31.75;
  //     else if(mfcc_buffer[i]<= -32)
      // mfcc_buffer[i] = -32;
  //     else
  //     mfcc_buffer[i] = mfcc_buffer[i]; 
  //      }
   for (int i =0;i<250;i++){
...
void KWS::average_predictions()
{
  ...
  for(int j=0;j<5;j++) {
    ...
    averaged_output[j] =  predictions[j];
  }   
